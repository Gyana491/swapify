...
// Route to mark listing as sold
router.patch('/listings/:id/mark-sold', authMiddleware, async (req, res) => {
    try {
        const listing = await Listing.findById(req.params.id);
        ...
        res.status(500).json({ 
            message: 'Error marking listing as sold',
            error: error.message
        });
    }
});
...
router.get('/search-listings', async (req, res) => {
    try {
        const { query, latitude, longitude, maxDistance = 50, category } = req.query;
